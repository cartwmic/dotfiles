...
| `adversarial-review` | analyze (when full_rigor) | 1. `adversarial-review-cycle` (this repo) | Run a single-model self-review of the 7 analyze checks. Emit a "DEGRADED MODE: no adversarial-review-cycle skill available" note in analyze.md's header. |
...
| `file-isolation-via-worktree` | apply (UNCONDITIONAL — worktree execution is the only model at every Scale) | 1. `pi-subagents` with `worktree: true` 2. inline `git worktree add` per the pre-flight commit + worktree convention | NONE — worktree isolation is mandatory; if neither binding works, apply ABORTS fail-closed (no in-tree execution exists). |
