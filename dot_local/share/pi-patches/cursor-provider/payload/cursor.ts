 ...
 *
 * chezmoi-pi-patch:cursor-provider v1
 */
...
function readMonthlyCapDollars(): number {
	const cursorSettings = (getSettings().providers as { cursor?: CursorCoreSettings }).cursor;
	const cap = cursorSettings?.monthlyCapDollars;
	return typeof cap === "number" && Number.isFinite(cap) && cap > 0 ? cap : DEFAULT_MONTHLY_CAP_DOLLARS;
}
...
export class CursorProvider extends BaseProvider {
	...
	async fetchUsage(deps: Dependencies): Promise<UsageSnapshot> {
		...
		const monthlyCapDollars = readMonthlyCapDollars();
		...
		const usedPercent = (usedDollars / monthlyCapDollars) * 100;
		const resetAt = plan.billingCycleEnd;

		const window: RateWindow = {
			label: `${formatDollars(usedDollars)}/${formatDollars(monthlyCapDollars)}`,
			usedPercent,
