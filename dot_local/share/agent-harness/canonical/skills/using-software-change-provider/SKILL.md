...
Drive the engine itself with the [using-loop-engine skill](../using-loop-engine/SKILL.md). Provider contract details: [README](references/README.md); frozen requirements: [docs/prd.md](references/docs/prd.md).
...
2. Author or revise the subject artifact in `artifact_root` using its template from [data/templates/](references/data/templates/). Material content changes require a revision bump — a bump retires all standing verdicts for that subject by design; keeping the revision asserts the edit was immaterial.
