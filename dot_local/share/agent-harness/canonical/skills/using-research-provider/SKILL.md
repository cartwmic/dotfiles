...
Drive engine CLI, JSON envelopes, and `show` / `append` / `event` with the [using-loop-engine skill](../using-loop-engine/SKILL.md). This skill is the research counterpart of the [using-software-change-provider skill](../using-software-change-provider/SKILL.md) and the [using-policy-document-provider skill](../using-policy-document-provider/SKILL.md): same engine loop, different artifacts, gates, and primary work. Provider contract: [README](references/README.md). Judging and adjudication: [reviewer protocol](references/data/reviewer-protocol.md). Artifact shapes: [templates](references/data/templates/).
...
Do the primary work outside Loop Engine, then record it in the subject artifacts. Author from [data/templates/](references/data/templates/).
