...
Drive engine commands and outcome handling with the [using-loop-engine skill](../using-loop-engine/SKILL.md). Provider contract: [README](references/README.md). Target constraints: [target guidance](references/data/target-guidance.md). Semantic evidence contract: [reviewer protocol](references/data/reviewer-protocol.md).
...
Choose [readme.json](references/data/readme.json) or [agents.json](references/data/agents.json). Copy it to a run-specific file; set `mode` to `draft` or `audit`; replace `target.path` with the absolute target path. Keep shipped `target.id` and `profile_version` unchanged unless intentionally authoring a custom profile.
