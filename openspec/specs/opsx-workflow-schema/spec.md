...
- **THEN** it SHALL state that every blind reviewer records `Attested HEAD:` (verbatim `git rev-parse HEAD` output, full 40-hex) and `Attested Path:` (verbatim `git rev-parse --show-toplevel` output) as its first findings-output lines before reviewing
