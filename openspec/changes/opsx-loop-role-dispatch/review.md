...
| Model Config | review_models + impl pinned | optional `author_model`/`review_models`/`impl_model`/`author_in_session` + `provider`/`*_provider` front-matter keys, resolved by `opsx models`; unset ⇒ session model |
...
- Scale M, `full_rigor: false` per frozen intent.md (armed-loop `opsx_dispatch` +
  tool mute; not a cross-capability migration).
- `delegation_mode: subagent-required` — Scale M gating code review needs blind
  subagent dispatch; Constitution IX skill edits apply.
- `code_review_mode` left absent (derives to gating-required at M, fail-closed).
- Spec Level: spec-anchored (default).
- `review_models` pinned in front-matter (user-layer review unset; need ≥2 for
  adversarial gating). Same pair as sibling `ease-opsx-models-ux`.
- `impl_model: cursor/composer-2.5` mirrors live env / user config.
...
- 2026-07-14 — review.md authored from schema template at loop start
  (openspec-loop); Scale/modes from frozen intent.md. Assumption: pin
