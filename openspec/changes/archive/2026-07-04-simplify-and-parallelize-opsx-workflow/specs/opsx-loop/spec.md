...
This capability consolidates the two retired capabilities **opsx-loop-kickoff** and
**opsx-loop-orchestration** into a single loop capability: the `/opsx-loop` pi extension
(guaranteed kickoff/continuation, `opsx gate` as the deterministic judge, budgets, stall
detection with the doneness gap-set ratchet, goal/conversation distillation, loop_hold
landing channel, worktree resolution) together with the single-orchestrator
drive-to-completion loop behind that gate (frozen-intent baseline, earliest-blocking
repair, blind delegated review verdicts, terminal-landing holds). Every requirement and
scenario from both retired capabilities is carried over verbatim, with NO behavior change
beyond two targeted edits: (a) tier vocabulary is XS|S|M plus an optional `full_rigor`
review.md front-matter flag (the former L/XL semantics map to "M + full_rigor"); and (b)
the doneness-dispatch requirement is tiered so that at Scale M WITHOUT `full_rigor` the
doneness question rides the blind code-review dispatch (same blind reviewer, verdict still
sealed to a separate `doneness.md`) while WITH `full_rigor` the independently dispatched
blind doneness judge is retained. The doneness gap-set ratchet stays valid at all tiers
>= M because `doneness.md` always exists. All hold/latch/stall/budget/locator/confirm
guards carry over untouched.
