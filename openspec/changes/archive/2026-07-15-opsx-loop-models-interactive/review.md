...
- Scale M, `full_rigor: false` per frozen intent.md (typical feature: Path B
  interactive `/opsx-loop models set` via pi TUI pickers + CLI write; includes
  CLI interactive per-model review thinking).
...
- 2026-07-14 — intent frozen after spikes: Path B (pi TUI pickers → CLI write);
  catalog via `pi --list-models`; custom substring-filtered SelectList;
  sequential review picks; full thinking parity. Path A TTY handoff deferred
  (no ExtensionUIContext stop/start; `pi.exec` ignores stdin).
- 2026-07-14 — human-authorized intent amend: review interactive thinking is
