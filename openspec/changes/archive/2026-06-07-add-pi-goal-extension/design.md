...
**D3 — Structured "capture" verdict via tool call, with text fallback.** Choice: pass a `submit_verdict` tool (`{met:boolean, reason:string}` TypeBox schema) to `complete()` and read the tool call's `arguments` — the claude-bridge "capture" path, reliable structured output across providers (probe-confirmed `stopReason=toolUse` for both bridge sonnet and deepseek). If no valid tool call is returned, fall back to tolerant free-text JSON parsing; on parse/auth failure default to `met:false` with the text as reason (spec `goal-loop.handle-evaluation-failure`). Alternatives: free-text JSON only (fragile — models wrap/format inconsistently); strict parse (deadlocks or false-completes). Rationale: never false-complete, never crash, avoid prompt-format brittleness. *(ADR test: 3/4 — promote candidate.)*
...
**D11 — Judge instruction in the user message, not `systemPrompt`.** Choice: send the full evaluator instruction (incl. the JSON-only directive) inside the user message. Alternatives: instruction in `systemPrompt` (the original — the claude-bridge provider runs the `claude` CLI which ignores `systemPrompt`, so the judge answered conversationally and never returned a verdict → "never met" for any goal). Rationale: provider-agnostic; a probe confirmed bridge sonnet returns proper `{"met":…}` JSON when the directive is in the user message. Transcript also falls back to session entries when the `agent_end` event carries no assistant text. *(ADR test: 3/4 — promote candidate.)*

**D10 — Stop the loop on interrupt/error via `stopReason`.** Choice: in `agent_end`, read the latest assistant message's `stopReason` from the event; if `"aborted"` (user interrupt) or `"error"`, deactivate the goal and do NOT re-inject; `/goal clear` also calls `ctx.abort()` when not idle. Alternatives: ignore stopReason (the original bug — the re-injected follow-up immediately fights the user's cancel, leaving a stuck working spinner); pause-instead-of-clear (no pause state exists; clear matches the user's "cancel" intent). Rationale: `StopReason` is `stop|length|toolUse|error|aborted`; an aborted/errored turn must not auto-continue. *(ADR test: 3/4 — promote candidate.)*
