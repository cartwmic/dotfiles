...
- [x] 1.1 Remove the retired turn-count resolvers and boundary helper:
...
- [x] 1.2 Add `resolveElideKeepPercent(raw)` (default 40, min 1, max 100) and
  ...
  - files_allowed:
      - dot_pi/agent/extensions/opsx-loop/helpers.ts
  - allow_new_files: false
- [x] 1.3 Add `resolveElideMaxKeepTokens(contextWindow, pctRaw)` = `pct% × window`
  ...
  - files_allowed:
      - dot_pi/agent/extensions/opsx-loop/helpers.ts
  - allow_new_files: false
- [x] 1.4 Add `tokenBudgetBoundary(messages, maxKeepTokens, bandTokens)` returning the
  ...
  - files_allowed:
      - dot_pi/agent/extensions/opsx-loop/helpers.ts
  - allow_new_files: false
- [x] 1.5 Adapt `elideToolResultBodies(messages, opts)` to consume the
...
- [x] 2.1 Rewire the `context` handler: resolve `maxKeep`/`band` from
  ...
  - allow_new_files: false
- [x] 2.2 Add the compaction-suppression guard: `if (session.compacting) return
  ...
  - files_allowed:
      - dot_pi/agent/extensions/opsx-loop/index.ts
  - allow_new_files: false
- [x] 2.3 Consume/clear `session.elided` on the distill (`awaitingChange`) continuation
...
- [x] 3.1 Replace the turn-count helper tests with token-budget tests:
