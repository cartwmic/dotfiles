...
The unified `opsx` multitool: a single executable that dispatches to `gate`, `models`,
and `loop` subcommands, replacing the three standalone `opsx-gate`, `opsx-models`, and
`opsx-loop` executables. Also defines the harness-neutral model-config WRITE surface.
...
|---|---|---|---|---|---|
| Unified Subcommand Dispatch | ✓ | ✓ | ✓ | ✓ | ✓ |
| Hard Cutover No Legacy Entrypoints | ✓ | ✓ | ✓ | ✓ | ✓ |
| Model Config Write Surface | ✓ | ✓ | ✓ | ✓ | ✓ |
