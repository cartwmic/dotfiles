...
Delta for the consolidation: the opsx-loop pi extension migrates its spawn calls to the
`opsx <subcommand>` form, fixes the argument-truncation and worktree-staleness bugs, adds
stall detection, and gains a `/opsx-loop models` config subcommand.

## MODIFIED Requirements

<!-- Requirement NAMES are retained verbatim as stable AC-ID keys (the slug
`opsx-loop-kickoff.opsx-gate-is-the-deterministic-judge` is cited in extension code/tests);
only normative BODIES migrate to the `opsx gate`/`opsx models` invocation. See design D6. -->
...
|---|---|---|---|---|---|
| Single-command guaranteed loop (MOD) | ✓ | ✓ | ✓ | ✓ | ✓ |
| opsx-gate is the deterministic judge (MOD) | ✓ | ✓ | ✓ | ✓ | ✓ |
| Loop exports resolved role models (MOD) | ✓ | ✓ | ✓ | ✓ | ✓ |
| Argument parsing preserves full input | ✓ | ✓ | ✓ | ✓ | ✓ |
| Stall detection stops the loop | ✓ | ✓ | ✓ | ✓ | ✓ |
