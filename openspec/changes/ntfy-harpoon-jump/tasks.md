...
- [x] 1.1 Author the remote notify wrapper that stamps the stable zellij pane id
...
- [x] 2.1 Author the phone `~/.ssh/config` `ControlMaster` snippet for the remote
...
- [x] 2.2 Extend `termux/sync.sh` to idempotently deliver the snippet: create the
...
- [x] 2.3 Document the ControlMaster snippet + push step in `termux/README.md`.
  - intent: feature
  - files_allowed:
      - termux/README.md
  - allow_new_files: false
...
- [x] 3.1 Verify idempotence + shell correctness: `bash -n` both scripts; assert
