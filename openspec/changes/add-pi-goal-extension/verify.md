...
## Test evidence
- `bun test` → **24 pass / 0 fail / 56 expect()** (parseVerdict; verdictFromToolArgs capture path; parseGoalArg incl. all clear aliases; shouldStopForBudget; decideAfterEvaluation incl. clarify-I1 met-wins; normalizeGoalConfig + resolveSetting precedence; lastAssistantInfo + isInterruptedStop for abort/error).
...
## UX fix (owner-reported)
- **`/goal status` set the goal to "status"; subcommands not discoverable** → `status` (and `?`) are now explicit status keywords in `parseGoalArg` (a real condition like "check status of x" still sets); added `getArgumentCompletions` so `status`/`clear` autocomplete after `/goal `. Command description now reads `/goal <condition> | /goal status | /goal clear`. `bun test` 25/0.
