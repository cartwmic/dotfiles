# Capability: opsx-loop-compaction-guard

<!-- Delta for merge-opsx-compact-percent-only: NEW capability closing the Lever A
spec gap. The opsx-loop extension's between-turns compaction guard is the operator's
sole compaction path (pi global auto-compaction is disabled). This change also
retires the legacy two-term trigger max(percent × window, absolute token floor):
`OPSX_COMPACT_AT_TOKENS` is removed and the trigger becomes percent-only. There is
no prior spec-of-record for this surface, so all requirements are ADDED. -->
...
|---|---|---|---|---|---|
| opsx-loop-compaction-guard.percent-only-compaction-trigger | [x] | [x] | [x] | [x] | [x] |
| opsx-loop-compaction-guard.default-on-with-explicit-off-only | [x] | [x] | [x] | [x] | [x] |
| opsx-loop-compaction-guard.garbage-falls-back-to-default | [x] | [x] | [x] | [x] | [x] |
| opsx-loop-compaction-guard.default-layers-above-elision-budget | [x] | [x] | [x] | [x] | [x] |
| opsx-loop-compaction-guard.policy-notify-describes-single-term | [x] | [x] | [x] | [x] | [x] |
