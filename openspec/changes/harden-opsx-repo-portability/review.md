...
- None — template defaults stand: Scale S (user picker 2026-07-04), full_rigor
  false, spec-anchored, worktree/code-review modes tier-derived (same-tree,
  advisory), loop_max_iterations 20. No sweep.txt declared: the retired literal
  (`main` hardcodes) is a common word with legitimate uses across shipped
  surfaces; an ERE would flood. The three functional sites are enumerated in
  intent.md Grounding instead.
