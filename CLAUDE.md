...
`adr/` holds decision records for the pi extensions this repo ships (repo-only;
not chezmoi-deployed). The OpenSpec/`opsx` workflow that originally produced
them was retired — see git history for the removed schema, CLI, loop extension,
and skills.
